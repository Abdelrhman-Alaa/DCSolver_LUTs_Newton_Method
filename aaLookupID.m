...
function ID = aaLookupID(LUTV, L, VGS, VDS, VSB, TEMP)
...
ID = (3 * step * dVGS.^2 - 2 * dVGS.^3) ./ step^3 .* ID2 ...
    + (step^3 - 3 * step * dVGS.^2 + 2 * dVGS.^3) ./ step^3 .* ID1 ...
    + dVGS.^2 .* (dVGS - step) ./ step^2 .* GM2 ...
    + dVGS .* (dVGS - step) .^ 2 ./ step^2 .* GM1;
